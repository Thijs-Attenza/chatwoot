...
function setup_chatwoot() {
  ...
  git clone https://github.com/Thijs-Attenza/chatwoot.git
  cd chatwoot
  git checkout "$BRANCH"
  bundle
  pnpm i
...
EOF
  ...
  echo -en "\n"
  read -rp 'Would you like to install Redis? (Answer no if you plan to use external services)(yes or no): ' install_pg_redis
  ...
  if [ "$install_pg_redis" != "no" ]; then
    ...
    echo "➥ 2/9 Skipping Redis installation."
  fi
